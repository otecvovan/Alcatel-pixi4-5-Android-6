...
extern u8 ctp_vendor_value;

static unsigned char CTPM_FW[] = {
     #include "Pixi4_5_4G_GL_6336U_ID87_V11_20160420_app.i"
};

static unsigned char CTPM_FW_GREEN[] = {
     #include "Pixi4_5_4G_6336U_IDa1_V11_20160408_app.i"
};
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		...
		if(0x87 == ctp_vendor_value)
			pbt_buf = CTPM_FW;
		if(0xa1 == ctp_vendor_value)
			pbt_buf = CTPM_FW_GREEN;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	*/
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		if(fts_updateinfo_curr.CHIP_ID==0x64 || fts_updateinfo_curr.CHIP_ID==0x36)
		{
...
int fts_ctpm_auto_upgrade(struct i2c_client *client, unsigned char tp_vendor)
{
        unsigned char uc_host_fm_ver;
        unsigned char uc_tp_fm_ver;
        unsigned char version_list_pixi45_6336_lcetron[] = {0x00,0x01,0x02,0x0a,0x10};
        unsigned char version_list_pixi45_6336_green[] = {0x00,0x01,0x0a,0x10};  //Keep the Truly tp firmware old version list that allows to be updated.
        int i,i_ret = -1;
        //unsigned char reg_val[2] = {0};
	fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
        if(tp_vendor == 0x87){
                uc_host_fm_ver = fts_ctpm_get_i_file_ver();
                printk("[FT6336] tp vendor is (0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
                for (i = 0; i < sizeof(version_list_pixi45_6336_lcetron)/sizeof(version_list_pixi45_6336_lcetron[0]); i++)
                {
                        if(uc_tp_fm_ver<uc_host_fm_ver)
                        {
                                if (uc_tp_fm_ver == version_list_pixi45_6336_lcetron[i])
                                {
                                        printk("[FT6336]  tp firmware have new version \n");
                        ...
                        {
                                printk("[FT6336] current tp firmware is the latest version!\n");
                                break;
                        }
                }
        }
        else if(tp_vendor == 0xa1){
                uc_host_fm_ver = fts_ctpm_get_i_file_ver();
                printk("[FT6336] tp vendor is TDT(0x%x),uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",tp_vendor, uc_tp_fm_ver, uc_host_fm_ver);
                for(i = 0;i < sizeof(version_list_pixi45_6336_green)/sizeof(version_list_pixi45_6336_green[0]); i++)
                {
                        if(uc_tp_fm_ver<uc_host_fm_ver)
                        {
                                printk("[FT6336] pixi45_green tp firmware old\n");
                                if(uc_tp_fm_ver == version_list_pixi45_6336_green[i])
                                {
                                        printk("[FT6336] pixi45_green tp firmware have new version\n");
                                        i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
                                        if(!i_ret)
                                                printk("[FT6336] pixi45_green tp firmware upgrade to new version 0x%x\n", uc_host_fm_ver);
                                        else
                                                printk("[FT6336] pixi45_green tp firmware upgrade failed ret=%d.\n", i_ret);
                                        break;
                                }
                        }
                        else
                ...
                printk("[FT6336] Unknown tp vendor(0x%x).\n", uc_tp_fm_ver);
                return -1;
        }
    return 0;
#if 0
