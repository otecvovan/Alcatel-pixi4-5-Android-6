...
#ifdef CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
extern u8 g_ProximityAssistanceMode;
#endif //CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
...
unsigned char msg22xx_yyyy_update_bin[94*1024] =
{
        #include "PIXI4-5-GL_EACH_0X5.206_20160630.h"
};
unsigned char msg22xx_xxxx_update_bin[94*1024] =
{
        #include "PIXI4-5-GL_EACH_0X5.206_20160630.h"
};
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        if ((nUpdateBinMinor > nMinor && nIsCompareVersion == 1) || (nIsCompareVersion == 0))
        {
            if (eSwId == MSG21XXA_SW_ID_XXXX)
            {
                ...
                {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                    _DrvFwCtrlStoreFirmwareData(&(msg22xx_xxxx_update_bin[i]), 1024);
#else // By one dimensional array
                    _DrvFwCtrlStoreFirmwareData(&(msg22xx_xxxx_update_bin[i*1024]), 1024);
#endif
                }
            ...
            else if (eSwId == MSG21XXA_SW_ID_YYYY)
            {
                ...
                {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                    _DrvFwCtrlStoreFirmwareData(&(msg22xx_yyyy_update_bin[i]), 1024);
#else // By one dimensional array
                    _DrvFwCtrlStoreFirmwareData(&(msg22xx_yyyy_update_bin[i*1024]), 1024);
#endif
                }
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        else //touch on screen
        {
            if (tInfo.nTouchKeyCode != 0)
            {
                ...
                else if (tInfo.nTouchKeyCode == 2) // TOUCH_KEY_MENU
                {
...
s32 DrvFwCtrlEnableProximity(void)
{
    ...
    {
        printk("*** Un-recognized chip type = 0x%x ***\n", g_ChipType);
        return -1;
    }
    ...
    if (rc > 0)
    {
        ...
        printk("Enable proximity detection success\n");
    }
    else
    {
        printk("Enable proximity detection failed\n");
    }
...
s32 DrvPowerSendFarEvent(void)
{
    ...
    if (rc > 0)
    {
        DBG("*** Enable send far event success ***\n");
    }
    else
    {
        DBG("*** Enable send far event failed ***\n");
        mutex_unlock(&g_Mutex);
        return -1;
    }
...
s32 DrvFwCtrlDisableProximity(void)
{
    ...
    if (rc > 0)
    {
        ...
        g_ProximityAssistanceMode = PROX_ASSIST_MODE_NORMAL;
#endif //CONFIG_ENABLE_PROXIMITY_ASSISTANCE_BY_GSENSOR
        g_EnableTpProximity = 0;

        printk("Disable proximity detection success\n");
    }
    else
    {
        printk("Disable proximity detection failed\n");
    }
...
s32 DrvFwCtrlSetProxAssistanceMode(ProxAssistanceMode_e nMode)
{
    u8 szDbBusTxData[4] = {0};
    s32 rc;
	printk("%s\n", __func__);
    ...
    if (rc > 0)
    {
        g_ProximityAssistanceMode = nMode;
        printk("Proximity Assistance Mode %x success\n", g_ProximityAssistanceMode);
    }
    else
    {
        printk("Proximity Assistance Mode %x failed\n", g_ProximityAssistanceMode);
    }
