...
/*[1101072] The capacity diaplay abnormally by zhuqiang at 20151217 bigen*/
int battery_vender_type=0; /* 1 BYD,2 VEKEN  */
 ...
 void fgauge_get_battery_vender_type(void)
{
  ...
  bat_id_vol = bat_id_vol/3;
   ...
   else if((bat_id_vol>=130)&&(bat_id_vol<=330))
   	{
...
/*[1101072] The capacity diaplay abnormally by zhuqiang at 20151217 end*/
...
/*[1101072] The capacity diaplay abnormally by zhuqiang at 20151217 begin*/
signed int fgauge_get_Q_max(signed short temperature)
{
	signed int ret_Q_max = 0;
	signed int low_temperature = 0, high_temperature = 0;
...
if (temperature <= batt_meter_cust_data.temperature_t1) {
		low_temperature = (-10);
		batt_meter_cust_data.q_max_neg_10 = Q_MAX_NEG_VEKEN_10;
		low_Q_max = batt_meter_cust_data.q_max_neg_10;
		high_temperature = batt_meter_cust_data.temperature_t1;
		batt_meter_cust_data.q_max_pos_0 = Q_MAX_POS_VEKEN_0;
		high_Q_max = batt_meter_cust_data.q_max_pos_0;
		...
		low_Q_max = batt_meter_cust_data.q_max_pos_0;
		high_temperature = batt_meter_cust_data.temperature_t2;
		batt_meter_cust_data.q_max_pos_25 = Q_MAX_POS_VEKEN_25;
		high_Q_max = batt_meter_cust_data.q_max_pos_25;
		...
		low_Q_max = batt_meter_cust_data.q_max_pos_25;
		high_temperature = batt_meter_cust_data.temperature_t3;
		batt_meter_cust_data.q_max_pos_50 = Q_MAX_POS_VEKEN_50;
		high_Q_max = batt_meter_cust_data.q_max_pos_50;
...
if (temperature <= batt_meter_cust_data.temperature_t1) {
		low_temperature = (-10);
		batt_meter_cust_data.q_max_neg_10_h_current = Q_MAX_NEG_10_H_VEKEN_CURRENT;
		low_Q_max = batt_meter_cust_data.q_max_neg_10_h_current;
		high_temperature = batt_meter_cust_data.temperature_t1;
		batt_meter_cust_data.q_max_pos_0_h_current = Q_MAX_POS_0_H_VEKEN_CURRENT;
		high_Q_max = batt_meter_cust_data.q_max_pos_0_h_current;
		...
		low_Q_max = batt_meter_cust_data.q_max_pos_0_h_current;
		high_temperature = batt_meter_cust_data.temperature_t2;
		batt_meter_cust_data.q_max_pos_25_h_current = Q_MAX_POS_25_H_VEKEN_CURRENT;
		high_Q_max = batt_meter_cust_data.q_max_pos_25_h_current;
		...
		low_Q_max = batt_meter_cust_data.q_max_pos_25_h_current;
		high_temperature = batt_meter_cust_data.temperature_t3;
		batt_meter_cust_data.q_max_pos_50_h_current = Q_MAX_POS_50_H_VEKEN_CURRENT;
		high_Q_max = batt_meter_cust_data.q_max_pos_50_h_current;
...
int fgauge_get_saddles(void)
{
	...
	return sizeof(battery_profile_veken_t2) / sizeof(BATTERY_PROFILE_STRUCT);
}
...
int fgauge_get_saddles_r_table(void)
{
	...
	return sizeof(r_profile_veken_t2) / sizeof(BATTERY_PROFILE_STRUCT);
}
...
{
if (temperature == batt_meter_cust_data.temperature_t0)
		return &battery_profile_veken_t0[0];

	if (temperature == batt_meter_cust_data.temperature_t1)
		return &battery_profile_veken_t1[0];

	if (temperature == batt_meter_cust_data.temperature_t2)
		return &battery_profile_veken_t2[0];

	if (temperature == batt_meter_cust_data.temperature_t3)
		return &battery_profile_veken_t3[0];

	if (temperature == batt_meter_cust_data.temperature_t)
		return &battery_profile_veken_temperature[0];
	return NULL;
}
...
{
if (temperature == batt_meter_cust_data.temperature_t0)
		return &r_profile_veken_t0[0];

	if (temperature == batt_meter_cust_data.temperature_t1)
		return &r_profile_veken_t1[0];

	if (temperature == batt_meter_cust_data.temperature_t2)
		return &r_profile_veken_t2[0];

	if (temperature == batt_meter_cust_data.temperature_t3)
		return &r_profile_veken_t3[0];

	if (temperature == batt_meter_cust_data.temperature_t)
		return &r_profile_veken_temperature[0];
...
/* modified by zhuqiang for PR1391816 */
/*modified by zhuqiang for PR1722427 */
int battery_write_file(const char* path, const void* buf, size_t size){
	...
	printk("zq write to %s,value=%s\n",path,wbuf);
	 set_fs(oldfs);
	 oldfs = get_fs();
    	 set_fs(get_ds());
	ret = vfs_fsync(filp, 0);
	if (ret < 0)
	printk("%s %i fail to sync write file %s\n", __func__, __LINE__, path);
	set_fs(oldfs);
        filp_close(filp, NULL);
    }
