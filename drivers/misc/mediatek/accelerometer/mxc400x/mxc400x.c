...
static long mxc400x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_GET_CALI:
		...
		sscanf(vec, "%x %x %x", &vec_buf[0], &vec_buf[1], &vec_buf[2]);

             /* change gsensor 20150608 end by xurd*/
                         if(copy_to_user(data, vec_buf, sizeof(vec_buf)))
                         {
                                 err = -EFAULT;
                                 break;
                         }
        ...
        case GSENSOR_IOCTL_GET_TEMP:
             ...
             break;
                case GSENSOR_IOCTL_READ_REG:
                        ...
                        if(mxc400x_i2c_read_block(client, test, &buf, 1));
                        {
                                GSE_ERR("i2c read error\n");
                        }
...
static int mxc400x_suspend(struct i2c_client *client, pm_message_t msg) 
{
#if 1//modify by lanying.he for gsnsor from 0 to 1
	 struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);	  
	 int err = 0; 
...
static int mxc400x_resume(struct i2c_client *client)
{
#if 1 //modify by lanying.he for gsensor  from 0 to 1
	 struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);		  
	int err=0;
	...
	if (obj == NULL) {
     ...
     mutex_lock(&mxc400x_mutex);
          //modify by lanying.he for gsensor begin
	
	 if(err == MXC400X_SetPowerMode(obj->client, true))   //if(err == mxc400x_init_client(client, 0))
	 //modify by lanying.he for gsensor end 
	 {
