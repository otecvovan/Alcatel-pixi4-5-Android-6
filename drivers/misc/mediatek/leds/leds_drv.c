...
static struct pinctrl *charge_led_pinctrl;
static struct pinctrl_state *charge_led_high;
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	/*[1221337] support leds device init by device tree modify by zhuqiang at 20151224 bigen */

	charge_led_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(charge_led_pinctrl)) {
		printk("Cannot find charge led pinctrl!");
		ret = PTR_ERR(charge_led_pinctrl);
	}

	charge_led_high = pinctrl_lookup_state(charge_led_pinctrl, "chareg_pins_default");
	if (IS_ERR(charge_led_high)) {
		ret = PTR_ERR(charge_led_high);
		printk("%s : init err, led_default\n", __func__);
	}
	pinctrl_select_state(charge_led_pinctrl, charge_led_high);

	/*[1221337] support leds device init by device tree modify by zhuqiang at 20151224 end */
...
static void mt65xx_leds_shutdown(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		case MT65XX_LED_MODE_PWM:
			LEDS_DRV_DEBUG("backlight control through PWM!!\n");
		...
		case MT65XX_LED_MODE_PMIC:
			LEDS_DRV_DEBUG("backlight control through PMIC!!\n");
			brightness_set_pmic(g_leds_data[i]->cust.data, 0, 0);
			break;
		case MT65XX_LED_MODE_CUST_LCM:
			LEDS_DRV_DEBUG("backlight control through LCM!!\n");
			((cust_brightness_set) (g_leds_data[i]->cust.data)) (0,
									     bl_div);
			break;
		case MT65XX_LED_MODE_CUST_BLS_PWM:
			LEDS_DRV_DEBUG("backlight control through BLS!!\n");
...
int led_brightness_shutdown(int level)//add by llf for factory reset
{
	mt65xx_leds_brightness_set(MT65XX_LED_TYPE_LCD,LED_OFF);
	mt65xx_leds_shutdown(NULL);
	return 0;

}
EXPORT_SYMBOL(led_brightness_shutdown);

/*[1221337] support leds device init by device tree modify by zhuqiang at 20151224 bigen */
#ifdef CONFIG_OF
static const struct of_device_id mt65xx_leds_device[] = {
	{.compatible = "mediatek,charge_led_gpio"},
	{},
};
#endif
static struct platform_driver mt65xx_leds_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   #ifdef CONFIG_OF
		   .of_match_table = of_match_ptr(mt65xx_leds_device),
		   #endif
		   },
