...
#define SUPPORT_I2C_BUS_NUM2        0 /*task799146 by zhanyong.yin at 20151103 begin */
#endif
...
/*task 1247762 add by yinzhanyong at 20151229 begin*/
UINT32 kdGetSensorInitFuncList_main(ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT **ppSensorList_main)
{
    if (NULL == ppSensorList_main)
    {
    PK_ERR("[kdGetSensorInitFuncList]ERROR: NULL ppSensorList\n");
    return 1;
    }
    *ppSensorList_main = &kdSensorList_main[0];
   // *ppSensorList = &kdSensorList_sub[0];
    return 0;
} /* kdGetSensorInitFuncList() */
UINT32 kdGetSensorInitFuncList_sub(ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT **ppSensorList_sub)
{
    if (NULL == ppSensorList_sub)
    {
    PK_ERR("[kdGetSensorInitFuncList]ERROR: NULL ppSensorList\n");
    return 1;
    }
    //*ppSensorList = &kdSensorList_main[0];
    *ppSensorList_sub = &kdSensorList_sub[0];
    return 0;
} /* kdGetSensorInitFuncList() */
/*task 1247762 add by yinzhanyong at 20151229 end*/
/*******************************************************************************
*iMultiReadReg
********************************************************************************/
...
/*task 1247762 add by yinzhanyong at 20151229 begin*/
    ...
    ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT *pSensorList_main = NULL;
    ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT *pSensorList_sub = NULL;
  ...
  // if (0 != kdGetSensorInitFuncList(&pSensorList))
   if (0 != kdGetSensorInitFuncList_main(&pSensorList_main))
    {
    PK_ERR("ERROR:kdGetSensorInitFuncList()\n");
    return -EIO;
    }
if (0 != kdGetSensorInitFuncList_sub(&pSensorList_sub))
    {
    PK_ERR("ERROR:kdGetSensorInitFuncList()\n");
    return -EIO;
    }
/*task 1247762 add by yinzhanyong at 20151229 end*/   
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		...
		/*task 1247762 add by yinzhanyong at 20151229 begin*/
 if (DUAL_CAMERA_SUB_SENSOR == g_invokeSocketIdx[i]) {
         if (NULL == pSensorList_sub[drvIdx[i]].SensorInit) {
            PK_ERR("ERROR:kdSetDriver()\n");
            return -EIO;
            }         
     pSensorList_sub[drvIdx[i]].SensorInit(&g_pInvokeSensorFunc[i]);

 }
 else{
            if (NULL == pSensorList_main[drvIdx[i]].SensorInit) {
            PK_ERR("ERROR:kdSetDriver()\n");
            return -EIO;
            }         
     pSensorList_main[drvIdx[i]].SensorInit(&g_pInvokeSensorFunc[i]);
    }        
			...
			spin_unlock(&kdsensor_drv_lock);
if (DUAL_CAMERA_SUB_SENSOR == g_invokeSocketIdx[i]) {
        memcpy((char *)g_invokeSensorNameStr[i], (char *)pSensorList_sub[drvIdx[i]].drvname, sizeof(pSensorList_sub[drvIdx[i]].drvname));
	//PK_XLOG_INFO("[kdSetDriver] :main [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_main[drvIdx[i]].drvname));
         PK_XLOG_INFO("[kdSetDriver] :sub [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_sub[drvIdx[i]].drvname));
	   //printk("[kdSetDriver] :main [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_main[drvIdx[i]].drvname));
        printk("[kdSetDriver] :sub [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_sub[drvIdx[i]].drvname));
}
else{
        memcpy((char *)g_invokeSensorNameStr[i], (char *)pSensorList_main[drvIdx[i]].drvname, sizeof(pSensorList_main[drvIdx[i]].drvname));
	PK_XLOG_INFO("[kdSetDriver] :main [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_main[drvIdx[i]].drvname));
         //PK_XLOG_INFO("[kdSetDriver] :sub [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_sub[drvIdx[i]].drvname));
	   printk("[kdSetDriver] :main [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_main[drvIdx[i]].drvname));
        //printk("[kdSetDriver] :sub [%d][%d][%d][%s][%d]\n", i, g_bEnableDriver[i], g_invokeSocketIdx[i], g_invokeSensorNameStr[i], sizeof(pSensorList_sub[drvIdx[i]].drvname));
...
/*task799146 modify by zhanyong.yin at 20151025 begin*/
				if (regSubVCAMD == NULL) {
					regSubVCAMD = regulator_get(sensor_device, "vgp1");
				}
/*task799146 modify by zhanyong.yin at 20151025 end*/
			} else {
