...
{
	char ictype[]="pixi5_ili9806e_fwvga_dsi_tdt";
	char moduletype[]="TDT";
	char batch[]="SUP1";
	char remark[]="0 x 0";
	char na[]="NA:NA:NA:0";
	...
	lcm_id = mt_get_gpio_in(GPIO_LCM_ID1)<<1 | mt_get_gpio_in(GPIO_LCM_ID2) ;

	if(lcm_id==1){//id == 0 is TDT 3 is BOE 2 is EACH
		if(plcm) 
			kfree(plcm);
		return sprintf(buf,"%s\n",na);
	}
	//printk("[LLF] lcm drv not null\n");
	if(lcm_id==0)// 0 mean tdt;
	{	
		char strid[]="TDT";
		char icname[]="ILI9806E";
		sprintf(ictype,"%s",icname);
		sprintf(moduletype,"%s",strid);
		sprintf(remark,"%dx%d",plcm->params->width, plcm->params->height);
		printk("[LLF]ictype :%s-------moduletype:%s ----------remark : %s\n",ictype,moduletype,remark);
		
	}else if(lcm_id==3){
		char strid[]="BOE";
		char strbatch[]="SUP2";
		char icname[]="JD9161BA";
		sprintf(ictype,"%s",icname);
		sprintf(moduletype,"%s",strid);
