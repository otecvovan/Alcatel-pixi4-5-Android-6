...
/* [1812021] Modified by peng.tian.hz at 20160318 for two type SD card compatible begin */
#include <mt_gpio.h>
#include <mach/gpio_const.h>

#define BOARD_ID_PIN GPIO95 | 0x80000000
#define SD_EINT_PIN  GPIO5 | 0x80000000
/* [1812021] Modified by peng.tian.hz at 20160318 for two type SD card compatible end */
						...
						unsigned long timeout)
{
	...
	if (opcode == MMC_READ_MULTIPLE_BLOCK) {
        ...
        /* PR815864 add by xiwu.peng for PowerOn-WP test begin from bin.song.hz */
        || (opcode == MMC_SEND_WRITE_PROT && (mmc_cmd_type(cmd) == MMC_CMD_ADTC))
        || (opcode == MMC_SEND_WRITE_PROT_TYPE && (mmc_cmd_type(cmd) == MMC_CMD_ADTC)) )
        /* PR815864 add by xiwu.peng for PowerOn-WP test end from bin.song.hz*/
		rawcmd |= (1 << 11);
...
int msdc_of_parse(struct mmc_host *mmc)
{
	...
	if (!mt_get_gpio_in(BOARD_ID_PIN)) {
		host->hw->flags |= MSDC_SD_NEED_POWER;
		host->hw->cd_level = MSDC_CD_HIGH;
	} else {
		host->hw->flags &= ~MSDC_SD_NEED_POWER;
		host->hw->cd_level &= ~MSDC_CD_HIGH;
		/* Eint5 pin need Pull-High  */
		mt_set_gpio_mode(SD_EINT_PIN, GPIO_MODE_00);
		mt_set_gpio_dir(SD_EINT_PIN, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(SD_EINT_PIN, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(SD_EINT_PIN, GPIO_PULL_UP);
	}
	/* [1812021] Modified by peng.tian.hz at 20160318 for two type SD card compatible begin */
...
/* Add by [peng.tian.hz] at [2015.09.26] for device info begin */
extern struct device* get_deviceinfo_dev(void);
static u8 *emmc_spt_tab[][4] = {
	/*:::*/
	"FE014E50314A39354B", "MT29TZZZ8D5BKFAH_125W_95K",	"Micron",	"8G+1G",
	"110100303038473730", "TYD0GH121661RA",			"Toshiba",	"8G+1G",
	"1F0100534850383038", "GPBD81S42MFLB_T3E",		"GCAI",		"8G+1G",
	"150100514E31534D42", "SP35BMCB7_S101T",		"Smart",	"8G+1G",
	NULL,
};

static size_t emmc_id_show(struct device *dev, 
		struct device_attribute *attr, const char *buf)
{
	...
	while(emmc_spt_tab[index][0] != NULL) {
		if(!strncmp(emmc_spt_tab[index][0], emmc_id, 18)) {
			return sprintf(buf, "%s:%s:SUP%d:%s\n", 
...
static size_t emmc_id_store(struct device *dev, 
...
/* Add by [peng.tian.hz] at [2015.09.26] for device info end */
...
/* Add by peng.tian.hz for device_info begin */
	if (host->hw->host_function == MSDC_EMMC) {
		create_memory_id_node();
	}
/* Add by peng.tian.hz for device_info end */
