	...
	struct mmc_card *oldcard)
{
	...
	if (card->ext_csd.cache_size > 0) {
		/* Modified by peng.tian.hz at 20160107 for change cache on timeout to 1s begin */
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				EXT_CSD_CACHE_CTRL, 1,
				1000);
		/* Modified by peng.tian.hz at 20160107 for change cache on timeout to 1s end */
		if (err && err != -EBADMSG)
			goto free_card;
